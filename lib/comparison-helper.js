...
const { readConfigFile, updateGenerateBaseTestsKeyInConfig, readPrivateKey } = require('./file-interactions');
...
const checkErrors = () => {
    ...
    if (!checkPrivateKey(baseOrg, destinationOrgs)) { ERROR(`Private key can't be blank if you are using oAuth`); return true }
...
const checkPrivateKey = (baseOrg, destinationOrgs) => {
    const creds = readConfigFile();
    const envCreds = creds.loginDetails[baseOrg]
    if (_.has(envCreds, 'oAuth.inUse') && creds.loginDetails[baseOrg].oAuth.inUse) return readPrivateKey() === '' ? false : true

    const destOrgCredsCheck = destinationOrgs.map(org => {
        const envCreds = creds.loginDetails[org]
        if (_.has(envCreds, 'oAuth.inUse') && creds.loginDetails[org].oAuth.inUse) return readPrivateKey() === '' ? false : true
        return true
    })
    return destOrgCredsCheck.includes(false) ? false : true;
}
