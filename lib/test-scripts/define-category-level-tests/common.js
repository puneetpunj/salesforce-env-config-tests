...
const { getPermissionSetsMetadata } = require('./permission-sets');
const reportValue = require('mochawesome/addContext')

const getMetadata = async (envName, category, objectName) => {
    if (category.includes('validation-rules')) return await getValidationRulesMetadata(envName, objectName)
    else if (category.includes("lov")) return await getLOVFieldsDataFromSalesforce(envName, objectName)
    else if (category.includes("permission-sets")) return await getPermissionSetsMetadata(envName, objectName)
}
...
const performAssertions = (actualDetails, test) => {
    ...
    expect(actualDetails).to.not.equal(`Permission Set ${test.inputData.name} Not found`, `Field -> "${test.inputData.name}" does not exist`)
    ...
    for (let i = 0; i < Object.keys(test.expectedOutput).length; i++) {
        ...
        if (Array.isArray(test.expectedOutput[expectedKey])) {
            ...
            const actualValue = actualDetails[expectedKey].toString().replace(/\n\s*/g, '\n')
            const expectedValue = test.expectedOutput[expectedKey].toString().replace(/\n\s*/g, '\n')
...
const mappers = {
    ...
    'permission-sets': (metadataDetails, expectedId) => {
        const recordDetails = metadataDetails.records.filter(item => item.Id == expectedId)[0]
        if (typeof (recordDetails) == 'undefined') return (`Permission Set ${expectedId} Not found`)
