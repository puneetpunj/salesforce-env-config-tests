const { suite, Test, suiteInstance } = require('../mocha-setup')
...
const { getLOVFieldsDataFromSalesforce, getMetadataDetails, getValidationRules } = require('../utiltities');
...
const buildObjectLevelTests = (testSuite, objectTests, metadataDetails, checkType) => {
    if (checkType == 'validation-rules') iterateTestsAndAddToMocha(objectTests, testSuite, metadataDetails, checkType)
    else if (checkType == 'lovs') defineLOVspecificTests(objectTests, testSuite, metadataDetails, checkType)
}
...
const defineLOVspecificTests = (testFileContent, testSuite, metadataDetails) => {
    Object.keys(testFileContent).filter(key => key !== 'TestSuiteName').forEach(recordType => {
        ...
        // console.log(testFileContent[recordType].RecordTypeTestSuiteName);
