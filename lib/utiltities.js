...
const GETLOVQueryEndpoint = (objectName, recordTypeDetails) => recordTypeDetails.totalSize > 0 ? `/services/data/v44.0/ui-api/object-info/${objectName}/picklist-values/${recordTypeDetails.records[0].Id}` : '';
...
const sampleCredentialsPresent = () => {
    ...
    const sampleKeys = Object.keys(creds).filter(i => creds[i].includes('sample'));
    return sampleKeys.length > 0 ? true : false
}


module.exports = { buildVLOVTestsforAllObjects, buildValidationRulesTestsforAllObjects, sampleCredentialsPresent }