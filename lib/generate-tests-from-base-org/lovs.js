...
const buildEndpointForLOVQuery = async (envName, objectName) => {
    ...
    for (let i = 0; i < recordTypeDetails.totalSize; i++) {
        LOVEndpoints[recordTypeDetails.records[i].Name] = await GETLOVQueryEndpoint(envName, objectName, recordTypeDetails.records[i].Id)
    }
    return LOVEndpoints
}

const GETLOVQueryEndpoint = async (envName, objectName, id) => {
    const version = await getVersionForCurrentEnv(envName)
    return `/services/data/v${version}/ui-api/object-info/${objectName}/picklist-values/${id}`;
}
