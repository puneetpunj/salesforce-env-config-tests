...
module.exports.printReportTable = () => {
    ...
    console.log(`
    ${TICK}  -> Tests for Validation Rules and List of Values are all Passed.
    ${CROSS}  -> Some of the Tests for either Validation Rules or List of Values are Failed. Check Detailed Report. 
    NA -> This object is not applicable for this Org based on config.json or there was no test generated from base org
    `)
}
    ...
    for (let i = 0; i < destOrgs.length; i++) {
        ...
        const allSuites = _.flatMap(orgSuites.map(categorySuites => {
            return categorySuites.suites.map(objectSuite => {
                const failures = _.max(objectSuite.suites.map(i => i.failures.length))
                const res = {}
                res[objectSuite.title] = failures || objectSuite.failures.length
                return res
            })
        }))
